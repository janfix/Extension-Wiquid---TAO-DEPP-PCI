...
function component(){
  ...
  this.rotate=function(){
    ...
    if(this.isRotated){
      ...
      console.log("replace à zero");
    }
    else{
      console.log("fait tourner");
      this.drawing.rotate(90, this.rotcx, this.rotcy);
      // Redressement spécial du M et replacement
      // Problème en passant par l'id je fige l'état de la page, donc ça coince avec plusieurs instance de Moteur.
      // La solution faire passer à cette fonction un flag qui permet de modifier la position du M.
      
      if(this.drawing[1].attrs.text){
        this.drawing[1].rotate(0, this.rotcx, this.rotcy);
        this.drawing[1].translate(5,5);
        } 
      console.log(this.drawing[1].attrs.text);
  ...
  // TODO: this should probably be renamed - could only be called in the beginning...
  // MATTER OF FACT, IS THIS EVEN USED ANYMORE?
...
function Point(obj,x,y){
  ...
  this.update=function(rotated,origin){
    ...
    // TODO: moving WHILE rotated is messed up.  The X and Y seem to be crossed when moving!
  ...
  this.rotate=function(origin){
    ...
    //console.log("  ROT input "+this.x+","+this.y+" origin: "+origin.x+","+origin.y);
    x=this.x-origin.x;
    y=this.y-origin.y;
    this.x = (x*costheta) - (y*sintheta)+origin.x;
    this.y = (x*sintheta) + (y*costheta)+origin.y;
    //console.log("  ROT output: "+this.x+","+this.y);
  };
...
wire.prototype.wireDragStop=function(){
  if(this.snapped){
    // TODO: are we using snappedObj for anything anymore?
    //this.snappedObj.addWireEnd(this);
  ...
  console.log("new:"+snap+" osnap:"+osnap);*/
  if(this.snappedObj !== this.osnappedObj && this.osnappedObj){
    //this.osnappedObj.removeWireEnd(this);
...
wire.prototype.wireEndGoto=function(x,y){
  //console.log("  ye hath moved me! "+x+","+y);
  this.attr("cx",x);
  this.attr("cy",y);
  this.obj.drawLine();
}
...
// todo: after making electrical routing work, I'll probably want to take
// another look at this.  still needed?
...
var paper;
var points = new Array(); // array of point objects with x,y,obj
...
*/  // get the size of the #simspace area
  paperSize = { w:parseInt($("#simspace").width()), h:parseInt($("#simspace").height()) };

  // create Raphael paper for display!
  paper = Raphael("simspace", paperSize.w, paperSize.h);
...
} // Fermeture de la fonction displaycircuit
